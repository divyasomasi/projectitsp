...
while(i<length(times))
    ...
    %if(find_max && data(times(i))>data(times(i+1)) && data(times(i))>data(times(i-1)))
        max_points(end+1)=times(i);
        find_max=false;
    
    else if(not(find_max) && data(times(i))<0.2)
        %if(not(find_max) &&  data(times(i))<data(times(i+1)) && data(times(i))<data(times(i-1)))
